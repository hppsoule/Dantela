...
const ChefChantierDashboard: React.FC = () => {
  ...
  const getStatusText = (statut: string) => {
    switch (statut) {
      case 'en_attente': 
        return language === 'fr' ? 'En attente' : 
               language === 'en' ? 'Pending' : 
               'Beklemede';
      case 'approuvee': 
        return language === 'fr' ? 'Approuvée' : 
               language === 'en' ? 'Approved' : 
               'Onaylandı';
      case 'rejetee': 
        return language === 'fr' ? 'Rejetée' : 
               language === 'en' ? 'Rejected' : 
               'Reddedildi';
      case 'en_preparation': 
        return language === 'fr' ? 'En préparation' : 
               language === 'en' ? 'In preparation' : 
               'Hazırlanıyor';
      case 'livree': 
        return language === 'fr' ? 'Livrée' : 
               language === 'en' ? 'Delivered' : 
               'Teslim edildi';
  ...
  const stats = {
    ...
    taux_approbation: demandes.length > 0 ? Math.round(((demandes.filter(d => d.statut !== 'en_attente' && d.statut !== 'rejetee').length) / demandes.length) * 100) : 0
  };
  ...
  const quickActions = [
    ...
    },
    {
      ...
      description: language === 'fr' ? 'Voir toutes mes demandes' : language === 'en' ? 'View all my requests' : 'Tüm taleplerimizi gör',
      icon: ClipboardList,
      color: 'from-purple-500 to-purple-600',
      bgColor: 'bg-purple-50',
      action: () => window.location.href = '/mes-demandes'
    },
    {
  ...
  return (
    <div className="space-y-4 sm:space-y-6 p-2 sm:p-4 lg:p-6">
      {/* Header avec informations du chef de chantier - Responsive */}
      <div className="bg-gradient-to-r from-teal-600 via-teal-700 to-teal-800 rounded-xl sm:rounded-2xl p-4 sm:p-6 lg:p-8 text-white relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          {/* Indicateurs de performance - Grid responsive */}
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4">
            ...
            <div className="bg-white bg-opacity-20 rounded-lg sm:rounded-xl p-3 sm:p-4 text-center backdrop-blur-sm">
              ...
              <p className="text-xs sm:text-sm text-teal-100">
                {language === 'fr' ? 'Total Demandes' : language === 'en' ? 'Total Requests' : 'Toplam Talep'}
              </p>
            </div>
            <div className="bg-white bg-opacity-20 rounded-lg sm:rounded-xl p-3 sm:p-4 text-center backdrop-blur-sm">
              ...
              <p className="text-xs sm:text-sm text-teal-100">
                {language === 'fr' ? 'Livrées' : language === 'en' ? 'Delivered' : 'Teslim Edildi'}
              </p>
            </div>
            <div className="bg-white bg-opacity-20 rounded-lg sm:rounded-xl p-3 sm:p-4 text-center backdrop-blur-sm">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        {[
          ...
          <div key={index} className="bg-white rounded-xl sm:rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden border border-slate-100 hover:scale-[1.02]">
      ...
      {/* Actions rapides - Mobile first design */}
      <div className="bg-white rounded-xl sm:rounded-2xl shadow-lg p-4 sm:p-6 border border-slate-100">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-6">
          <div className="mb-4 sm:mb-0">
            ...
            <p className="text-sm sm:text-base text-slate-600">
              {language === 'fr' ? 'Accès direct aux fonctionnalités principales' : 
               language === 'en' ? 'Direct access to main features' : 
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          {quickActions.map((action, index) => (
            ...
            >
              <div className={`absolute top-0 right-0 w-16 sm:w-20 h-16 sm:h-20 bg-gradient-to-br ${action.color} opacity-10 rounded-full -translate-y-8 sm:-translate-y-10 translate-x-8 sm:translate-x-10 group-hover:scale-110 transition-transform duration-300`}></div>
      ...
      {/* Historique des commandes - Section principale */}
      <div className="bg-white rounded-xl sm:rounded-2xl shadow-lg border border-slate-100">
        <div className="p-4 sm:p-6 border-b border-slate-200 bg-gradient-to-r from-blue-50 to-teal-50">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div className="mb-4 sm:mb-0">
              <h2 className="text-xl sm:text-2xl font-bold text-slate-900 mb-1 sm:mb-2">
                {language === 'fr' ? 'Historique de mes Commandes' : 
                 language === 'en' ? 'My Orders History' : 
                 'Sipariş Geçmişim'}
              </h2>
              <p className="text-sm sm:text-base text-slate-600">
                {language === 'fr' ? 'Suivez l\'état de toutes vos demandes de matériaux' : 
                 language === 'en' ? 'Track the status of all your material requests' : 
          ...
          {demandes.length === 0 ? (
            <div className="text-center py-8 sm:py-12">
              ...
              <h3 className="text-lg sm:text-xl font-medium text-slate-900 mb-2">
                {language === 'fr' ? 'Aucune demande trouvée' : 
                 language === 'en' ? 'No requests found' : 
                 'Talep bulunamadı'}
              </h3>
              <p className="text-sm sm:text-base text-slate-600 mb-4">
                {language === 'fr' ? 'Vous n\'avez pas encore fait de demande de matériaux' : 
                 language === 'en' ? 'You haven\'t made any material requests yet' : 
              ...
              >
                {language === 'fr' ? 'Faire ma première demande' : 
                 language === 'en' ? 'Make my first request' : 
                 'İlk talebimi yap'}
              </button>
            </div>
          ) : (
            <div className="space-y-3 sm:space-y-4">
              {demandes.map(demande => (
                <div key={demande.id} className="bg-gradient-to-r from-slate-50 to-slate-100 rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-slate-200 hover:shadow-lg transition-all duration-300 hover:scale-[1.01]">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 text-sm sm:text-base text-slate-600 mb-3 sm:mb-4">
                        <div className="flex items-center space-x-2">
                          ...
                          <span>
                            {language === 'fr' ? 'Demandé le:' : 
                             language === 'en' ? 'Requested:' : 
                             'Talep tarihi:'} {new Date(demande.date_demande).toLocaleDateString()}
                          </span>
                        </div>
                        <div className="flex items-center space-x-2">
                          <Truck className="w-4 h-4 text-slate-500" />
                          <span>
                            {language === 'fr' ? 'Livraison:' : 
                             language === 'en' ? 'Delivery:' : 
                             'Teslimat:'} {new Date(demande.date_livraison_souhaitee).toLocaleDateString()}
                          </span>
                        </div>
                        <div className="flex items-center space-x-2">
                          <Package className="w-4 h-4 text-slate-500" />
                          <span>
                            {demande.nombre_items} {language === 'fr' ? 'matériau(x)' : 
                          ...
                          <div className="flex items-start space-x-2">
                            ...
                            <div>
                              <p className="text-sm font-medium text-blue-700">
                                {language === 'fr' ? 'Mon commentaire:' : 
                                 language === 'en' ? 'My comment:' : 
                                 'Yorumum:'}
                              </p>
                          ...
                          <div className="flex items-start space-x-2">
                            ...
                            <div>
                              <p className="text-sm font-medium text-gray-700">
                                {language === 'fr' ? 'Réponse du magazinier:' : 
                                 language === 'en' ? 'Warehouse response:' : 
                                 'Depo yanıtı:'}
                              </p>
                              <p className="text-sm text-gray-600">{demande.commentaire_magazinier}</p>
                              {demande.valideur_nom && demande.date_validation && (
                                <p className="text-xs text-gray-500 mt-1">
                                  {language === 'fr' ? 'Par' : language === 'en' ? 'By' : 'Tarafından'} {demande.valideur_nom} - {new Date(demande.date_validation).toLocaleDateString()}
                                </p>
                              )}
                            </div>
                          </div>
                  ...
                  <div className="mt-4 pt-4 border-t border-slate-200">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-2 sm:space-y-0">
                      <div className="text-xs sm:text-sm text-slate-500">
                        {language === 'fr' ? 'Créé le' : 
                         language === 'en' ? 'Created on' : 
                         'Oluşturulma'} {new Date(demande.date_demande).toLocaleDateString()} {language === 'fr' ? 'à' : 'at'} {new Date(demande.date_demande).toLocaleTimeString()}
                      </div>
                      ...
                      {demande.statut === 'livree' && (
                        <div className="flex items-center space-x-1 text-green-600">
                          ...
                          <span className="text-xs sm:text-sm font-medium">
                            {language === 'fr' ? 'Commande terminée' : 
                             language === 'en' ? 'Order completed' : 
                             'Sipariş tamamlandı'}
                          </span>
                        </div>
                      )}
      ...
      {/* Section d'encouragement - Mobile friendly */}
      {stats.total > 0 && (
                ...
                <h3 className="text-lg sm:text-xl font-bold">
                  {stats.taux_approbation >= 80 ? 
                    (language === 'fr' ? 'Excellent Travail !' : 
                     language === 'en' ? 'Excellent Work!' : 
                     'Mükemmel İş!') :
                    (language === 'fr' ? 'Continuez vos efforts !' : 
                     language === 'en' ? 'Keep up the good work!' : 
                     'İyi çalışmaya devam edin!')
                  }
                </h3>
              </div>
              <p className="text-sm sm:text-base text-emerald-100 leading-relaxed">
                {language === 'fr' ? `Vous avez un taux d'approbation de ${stats.taux_approbation}% avec ${stats.total} demandes créées.` : 
                 language === 'en' ? `You have a ${stats.taux_approbation}% approval rate with ${stats.total} requests created.` : 
            ...
            <div className="p-4 sm:p-6 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-teal-50">
              <div className="flex items-center justify-between">
                <h2 className="text-lg sm:text-xl font-semibold text-gray-900">
                  {language === 'fr' ? 'Détails de la Demande' : 
                   language === 'en' ? 'Request Details' : 
                   'Talep Detayları'} - {selectedDemande.numero_demande}
                </h2>
                <button
                  onClick={() => {
                  ...
                  <div>
                    <strong>
                      {language === 'fr' ? 'Date de demande:' : 
                       language === 'en' ? 'Request date:' : 
                       'Talep tarihi:'}
                    </strong> {new Date(selectedDemande.date_demande).toLocaleDateString()}
                  </div>
                  <div>
                    <strong>
                      {language === 'fr' ? 'Livraison souhaitée:' : 
                       language === 'en' ? 'Desired delivery:' : 
                       'İstenen teslimat:'}
                    </strong> {new Date(selectedDemande.date_livraison_souhaitee).toLocaleDateString()}
                  </div>
                  {selectedDemande.valideur_nom && (
                    <div className="sm:col-span-2">
                      <strong>
                        {language === 'fr' ? 'Validé par:' : 
                         language === 'en' ? 'Validated by:' : 
                         'Onaylayan:'}
                      </strong> {selectedDemande.valideur_nom} - {new Date(selectedDemande.date_validation).toLocaleDateString()}
                    </div>
                  )}
                </div>
              </div>

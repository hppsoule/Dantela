/*
  # Création complète de la table matériaux
  ...
  3. Données de démonstration
    - Matériaux de construction typiques
    - Codes produits uniques
    - Stock et fournisseurs réalistes
*/

-- Extension pour générer des UUID (si pas déjà créée)
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- Fonction pour mettre à jour updated_at automatiquement (si pas déjà créée)
...
-- Créer la table materiaux complète
...
-- Insérer des matériaux de démonstration
...
-- Affichage des matériaux créés pour vérification
...
-- Affichage du résumé
SELECT 
    'Résumé des matériaux:' as info,
    COUNT(*) as total_materiaux,
    SUM(stock_actuel) as total_unites_stock,
    COUNT(CASE WHEN stock_actuel <= stock_minimum THEN 1 END) as materiaux_stock_faible,
    COUNT(DISTINCT fournisseur) as nombre_fournisseurs,
    COUNT(DISTINCT categorie_id) as nombre_categories
FROM materiaux;